...
def insert_control_points(G_, control_points, max_distance_meters=10,
                          allow_renaming=True,
                          n_nodes_for_kd=1000, n_neighbors=-1,
                          x_coord='x', y_coord='y',
...
def _clean_sub_graphs(G_, min_length=80, max_nodes_to_skip=100,
    ...
    for G_sub in sub_graphs:
        ...
        if len(G_sub.nodes()) > max_nodes_to_skip:
            ...
            if max_len <= 0:
                print('bad nodes with min_length: ', max_len, min_length)
...
#                                        verbose=verbose)
    ...
    #print('all_pairs_lengths_gt_prime: ', all_pairs_lengths_gt_prime[all_pairs_lengths_gt_prime.keys()[1]])
...
def single_path_metric(len_gt, len_prop, diff_max=1):
    ...
    if len_gt <= 0:
        ...
        #print('diff_max, diff_raw: ', diff_max, diff_raw)
...
def path_sim_metric(all_pairs_lengths_gt, all_pairs_lengths_prop,
    ...
    t0 = time.time()
    print('gt_start_nodes_set: ', gt_start_nodes_set)
    print('prop_start_nodes_set: ', prop_start_nodes_set)
    ...
    num_missing_start, num_match, num_missing_end = 0, 0, 0
    for start_node in good_nodes:
        ...
        # else get proposed paths
            ...
            print('end_nodes_gt_set: ', end_nodes_gt_set)
            ...
            #for end_node in end_nodes_prop_set:
                # for end_node, len_gt in paths.iteritems():

                len_gt = paths[end_node]
                #len_gt = paths_prop[end_node]
                # skip if too short
    ...
    if normalize:
        ...
        print('norm: ', len(diffs))
        print('num_missing_start, num_match, num_missing_end: ', num_missing_start, num_match, num_missing_end)
    else:
        C = diff_tot
...
def compute_apls_metric(all_pairs_lengths_gt_native,
    ...
    print("C_gt_onto_prop:", C_gt_onto_prop)
    
    return C_gt_onto_prop
...
#            #########################
    ...
    if test_method == 'gt_json_prop_json':
        ...
        for f in name_list:
            ...
            # skip empty ground truth graphs
            if G_gt_init==None or len(G_gt_init.nodes()) == 0:
                continue
...
def main():
    ...
    args.gt_min_subgraph_length = 0
    args.prop_subgraph_filter_weight = 'length_pix'
    args.prop_min_subgraph_length = 0  # GSD = 0.3
